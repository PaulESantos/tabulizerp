...
[![CRAN
status](https://www.r-pkg.org/badges/version/tabulizerp)](https://CRAN.R-project.org/package=tabulizerp)
[![](https://badges.ropensci.org/42_status.svg)](https://github.com/ropensci/onboarding/issues/42)
<!-- badges: end -->

**tabulizerp** provides R bindings to the [Tabula java
...
tabulizerp depends on [rJava](https://cran.r-project.org/package=rJava),
...
``` r
pak::pkg_install(c("PaulESantos/tabulizerp",
...
``` r
library("tabulizerp")
f <- system.file("examples", "data.pdf", package = "tabulizerp")
...
memory constraints, leading to a
`java.lang.OutOfMemoryError: Java heap space` error message. Memory can
be increased using `options(java.parameters = "-Xmx16000m")` set to some
...
the path to your Java installation (e.g.,
`C:\Program Files\Java\jdk1.8.0_92`). This can be done:

1.  within R using
    `Sys.setenv(JAVA_HOME = "C:/Program Files/Java/jdk1.8.0_92")` (note
    slashes), or
2.  from command prompt using the `setx` command:
    `setx JAVA_HOME C:\Program Files\Java\jdk1.8.0_92`, or
3.  from PowerShell, using the .NET framework:
    `[Environment]::SetEnvironmentVariable("JAVA_HOME", "C:\Program Files\Java\jdk1.8.0_92", "User")`,
    or
4.  from the Start Menu, via
    `Control Panel » System » Advanced » Environment Variables`
    ([instructions here](http://superuser.com/a/284351/221772)).
...
-   On Windows, make sure you have permission to write to and install
    ...
    “Run as administrator” when launching R (again, from the right-click
    context menu).
