...
pdffile <- system.file("examples", "data.pdf", package = "tabulizerp")

test_that("File localization does not overwrite existing temporary file", {
  tmp <- tempfile(fileext = ".pdf")
  file.copy(from = pdffile, to = tmp)
  localfile <- tabulizerp:::localize_file(tmp, copy = TRUE)
...
test_that("Repeat areas", {
    a1 <- make_area(list(c(0, 0, 10, 10)), pages = 1)
    a2 <- tabulizerp:::make_area(list(c(0, 0, 10, 10)), pages = 1:2)
    a3 <- tabulizerp:::make_area(list(c(0, 0, 10, 10)), npages = 2)
...
test_that("make_area errors", {
    expect_error(tabulizerp:::make_area(1L))
    expect_error(tabulizerp:::make_area(list(c(0,0,10,10), c(0,0,10,10)), pages = 1))
    expect_error(tabulizerp:::make_area(list(c(0,0,10,10), c(0,0,10,10)), npages = 3))
})

test_that("Repeat columns", {
    c1 <- tabulizerp:::make_columns(list(c(0, 0, 10, 10)), pages = 1)
    c2 <- tabulizerp:::make_columns(list(c(0, 0, 10, 10)), pages = 1:2)
    c3 <- tabulizerp:::make_columns(list(c(0, 0, 10, 10)), npages = 2)
...
test_that("make_columns errors", {
    expect_error(tabulizerp:::make_columns(1L))
    expect_error(tabulizerp:::make_columns(list(c(0,1,2,3), c(0,1,2,3)), pages = 1))
    expect_error(tabulizerp:::make_columns(list(c(0,1,2,3), c(0,1,2,3)), npages = 3))
})
